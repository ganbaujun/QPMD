...
# 获取数据
matrix = get_dataset1("C:\\Users\\ASUS\\Desktop\\lock_exchange_160")

vtu_num = get_vtu_num("C:\\Users\\ASUS\\Desktop\\lock_exchange_160")

vtu_num_  = 2**int(np.log2(vtu_num)) + 2
print(vtu_num_)
snapshot_u = matrix[:, 0:]
# 步骤1: 对数据进行归一化
...
mydmap_u = dm.DiffusionMap.from_sklearn(n_evecs=n_evecs, k = 50, epsilon=45, alpha=1, neighbor_params=neighbor_params, metric=geodesic_metric_u)#16(14.6) 21(14.9)


# 使用 Dmap 拟合数据
dmap_u = mydmap_u.fit_transform(delta_u[:, 2:vtu_num_].T)
...
# 使用多项式特征矩阵拟合目标矩阵
model1.fit(X_poly_u, delta_u[:, 2:vtu_num_].T)
...
max_S = np.max(np.max(original_u[:,2:]))
Sp_q = Sp_q*max_S
print(Sp_q)
...
# 拼接预测结果
    velocity_predicted = predicted_new_point1.T
    velocity_predicted = torch.tensor(velocity_predicted).type(torch.float)
    file_path1 = f"C:\\Users\\ASUS\\Desktop\\lock_exchange_160\\lock_exchange_{vtu_num_ + k}.vtu"
    # 读取相应的 mesh 文件
    ...
    print(np.linalg.norm(velocity_predicted-velocity))
    #print(mean_squared_error(velocity_predicted, velocity))
...
# 将新的velocity替换到mesh的数据中
    mesh.point_data["Temperature"] = velocity_predicted.numpy()  # 只替换第一列（u方向）

    # 将修改后的mesh保存为新的vtu文件
    output_file_path = f"C:\\Users\\ASUS\\Desktop\\lock_exchange_160\\锁交换预测数据Q1_6\\lock_exchange_{vtu_num_ + k}.vtu"
    meshio.write(output_file_path, mesh)

    mesh.point_data["Temperature"] = velocity_predicted.numpy()-velocity.numpy()  # 只替换第一列（u方向）

    # 将修改后的mesh保存为新的vtu文件
    output_file_path = f"C:\\Users\\ASUS\\Desktop\\lock_exchange_160\\锁交换误差数据Q1_6\\lock_exchange_{vtu_num_ + k}.vtu"
    meshio.write(output_file_path, mesh)
